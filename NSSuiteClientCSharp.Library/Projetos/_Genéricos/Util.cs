...
namespace NSSuiteClientCSharp.Projetos.Genéricos
{
    internal static class Util
    {
        ...
        public static string VerificaAssinaturaA3(string conteudo, string tpConteudo, string cnpjEmitente, string nodoAssinatura)
        {
            if (!tpConteudo.ToLower().Equals("xml"))
                throw new Exception("Para fazer a assintaura voce deve enviar um XML do projeto que deseja emitir!");

            X509Certificate2 cert = BuscaCertificado(cnpjEmitente.Trim());

            if (cert == null)
                throw new Exception("Certificado Digital não encontrado");
            return AssinaXML(conteudo.Trim(), nodoAssinatura, cert);
        }
        ...
        internal static void CriaCaminhoDiretorio(string caminho)
        {
            try
            {
            ...
            {
                Util.GravarLinhaLog("[CRIAR_DIRETORIO]" + caminho);
                Util.GravarLinhaLog(ex.Message);
                throw new Exception("Erro: " + ex.Message);
            }
        }

        public static string VerificaConteudoParaAssituraXML(bool a3, string conteudo, string tpConteudo, string cnpjEmitente, string nodoAssinatura)
        {
